...
for N, filename in enumerate(filenames):
    ...
    print('running rwpf_demo on', basename, f'({N+1} of {len(filenames)})')
    ...
    ax[1,1].axis('off')
    ax[1,1].set_title(rf'   fixed $p={P_THRESHOLD}$', loc='left')
    ...
    fig.savefig(f'./output/RWPFDEMO/{basename}_m.png')
    ...
    if (N % 25 == 0) and (N > 0):
        ...
        with open(f'rwpf_demo_scores_{N//25}.json', 'w') as f:
            json.dump(run_data, f)

with open(f'rwpf_demo_scores_all.json', 'w') as f:
        json.dump(run_data, f)
